...
const LESSON_DEFINITIONS: TutorialLessonDefinition[] = [
  ...
  },
  {
    ...
    intro: lt(
      'Create a figure named workflow_diagram, reference it with ref_fig, cite the Airalogy arXiv entry, and define that reference in a refs block.',
      '创建名为 `workflow_diagram` 的图，用 `ref_fig` 引用它，引用 Airalogy arXiv 文献，并在 `refs` 代码块中定义该文献。',
    ),
    focus: [
      lt('Figures', '图'),
      lt('Citations', '文献引用'),
    ...
    solution: lt(
      lines([
        '## Evidence',
        '',
        'The workflow is summarized in {{ref_fig|workflow_diagram}} and follows the Airalogy paper {{cite|yang2025airalogyaiempowereduniversaldata}}.',
        '',
        '```fig',
        'id: workflow_diagram',
        `src: "${DEMO_FIGURE_SRC}"`,
      ...
      lines([
        '## 证据',
        '',
        '流程见 {{ref_fig|workflow_diagram}}，并参考 Airalogy 论文 {{cite|yang2025airalogyaiempowereduniversaldata}}。',
        '',
        '```fig',
        'id: workflow_diagram',
        `src: "${DEMO_FIGURE_SRC}"`,
    ...
    hints: [
      lt('Figures are fenced ```fig blocks with required id/src and optional title/legend.', '图使用 fenced ` ```fig ` 代码块；`id` 和 `src` 必填，`title` 和 `legend` 可选。'),
    ...
    checks: [
      ...
      {
        id: 'citation-marker',
        label: lt('Cite the Airalogy arXiv entry in the text', '在正文中引用 Airalogy arXiv 文献'),
        evaluate: state => state.fields?.cite?.includes('yang2025airalogyaiempowereduniversaldata') ?? false,
      },
      {
        id: 'reference-entry',
        label: lt('Define the Airalogy arXiv BibTeX entry in a refs block', '在 `refs` 代码块中定义 Airalogy arXiv BibTeX 条目'),
        evaluate: state => findReference(state.fields, 'yang2025airalogyaiempowereduniversaldata')?.title === 'Airalogy: AI-empowered universal data digitization for research automation',
      },
      {
        id: 'rendered-refs',
