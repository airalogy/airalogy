...
```aimd
{{step|<step_id>, <step_level>[, duration="<duration>"][, timer="elapsed|countdown|both"]}}
```

- `<step_level>`为Step的层级，为整数`1`, `2`, `3`。之所以约束最多为3层，是为了保证Step的层级不会过深，以保证前端的易读性。
- 如果用户没有主动定义`<step_level>`，则默认为`1`。
- `duration`用于声明该Step的预计时长，推荐使用紧凑单位串，如`30s`、`10m`、`1h30m`、`2d4h`。
- `timer`用于声明该Step的计时展示模式，可选`elapsed`、`countdown`、`both`。默认是`elapsed`。
...
对于每个Step，前端通常都会提供备注能力；当备注为空时，可以采用渐进披露的方式，仅在用户主动展开或已有备注内容时显示备注编辑区域。

#### 预计时长

对于某些Step，Protocol设计者可能希望显式声明其预计耗时，以便前端后续做总时长估算、Step计时提示等。这可以通过在Step模板中使用参数`duration`来实现。

```aimd
{{step|<step_id>, <step_level>, duration="<duration>"}}
```
...
{{step|step_1}}
{{step|step_1_1, 2, duration="10m"}}
{{step|step_2, 1, duration="1h30m"}}
{{step|step_3}}
```

其中，`duration`在内部会被规范化为毫秒数，以便后续进行协议总时长估算和相关交互处理；同时会保留原始字符串表示，方便展示与回写。

#### 计时展示模式

如果某个Step不仅仅需要正向计时，还希望前端直接显示剩余时间，可以在`duration`的基础上增加参数`timer`。

```aimd
{{step|<step_id>, <step_level>, duration="<duration>", timer="elapsed|countdown|both"}}
```

- `elapsed`：默认模式，只显示正向累计耗时。
- `countdown`：以`duration`为目标时长，优先显示剩余时间；归零后进入 overtime。
- `both`：主显示倒计时，次显示已耗时。

例：

```aimd
{{step|step_1, duration="10m"}}
{{step|step_2, duration="30s", timer="countdown"}}
{{step|step_3, duration="5m", timer="both"}}
```
...
AIMD模板参数中被单引号或双引号包裹的内容会按普通字符串处理。也就是说，`checked_message`、`title`、`description`等字符串参数内部即使出现`{{var|...}}`、`{{ref_var|...}}`或`}}`，也不会被当作新的AIMD模板解析或渲染，而是作为原始文字保留。
...
在上面的例子中，`checked_message`的实际内容是普通字符串`Reading unstable after {{ref_var|stable_wait_s}} s; check electrode and sample temperature.`。如果需要让变量引用参与正文渲染，应把引用写在模板外部的AIMD正文中：
