...
| `stock-fundamental-analysis-assistant` | AI-assisted stock fundamental analysis | Engine | Enter a stock ticker and company name, configure an OpenAI-compatible web-search model, and organize public filings, business segments, financial quality, valuation comparisons, risks, and a research report draft. |
| `monitoring-site-flow-graph-3d-print` | Monitoring site physical graph and 3D print parameters | Engine | Generate a physics-constrained directed graph from site latitude, longitude, and elevation, then output node, edge, scale, and structure parameters for 3D printing. |
