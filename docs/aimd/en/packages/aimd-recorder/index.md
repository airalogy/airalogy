...
- Entity-reference controls for `EntityRef` and `list[EntityRef]` when the host provides `entityResolvers`; hosts can build these from parsed `connectors` metadata with `@airalogy/aimd-core/utils`.
