...
When missing figure IDs are generated during download, the status area reports how many IDs were generated and written into the editor source.
...
When downloading, the demo first normalizes all closed `fig` blocks. A figure with `src` but no ID receives a document-unique ID derived from its source filename, and the normalized text is written back into the editor before output; existing IDs are preserved, while duplicate IDs or missing sources stop formal output. Incomplete draft blocks remain untouched by automatic draft saving. The demo then chooses the exchange format from the current content: if there are no protocol-local files, it downloads a plain `.aimd` text file; if local images have been uploaded, it downloads a `kind: "protocol"` `.aira` archive containing both `protocol.aimd` and those local files. The demo derives a safe, readable resource path from the uploaded local filename. For example, `Reaction Rate Curve.png` is packaged as `files/reaction-rate-curve.png`, `图片.png` as `files/图片.png`, and `实验结果：第1组.png` as `files/实验结果-第1组.png`; if the filename cannot be converted into a safe path, the demo falls back to an automatic name such as `files/uploaded-figure-1.png`. The image insertion popover first lets users choose “local image” or “image URL,” then shows the matching input. `title` and `legend` are optional figure metadata shared by both modes and omitted when left blank. `legend` is a caption, not a filename note.

When importing, the demo also accepts `kind: "records"` `.aira` archives that embed their referenced Protocols. Records are resolved against the matching Protocol version and open directly in a table view. Users can filter by Protocol fields, open one Record, or select two to four Records and switch to the comparison view. The table and comparison use the shared Record views from `@airalogy/aimd-renderer`, while the single-Record display continues to use `@airalogy/aimd-recorder`, so the demo and Platform follow the same field interpretation and value-display rules.
