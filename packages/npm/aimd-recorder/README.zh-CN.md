...
Schema 契约同时支持 engine 的 `vars` / `steps` / `checks` 键和兼容用的 `research_variable` / `research_step` / `research_check` 别名，覆盖必填、类型、格式、pattern、枚举、数值边界、数组、对象及内建类型约束。对 Schema 标记为 required 的 Recorder 输入，即使内存 Record 中已有该属性，空字符串和空数组仍会判为未填。Recorder 会先归一化上传文件等结构化值，再执行 Schema 校验。没有传 Schema 时，Recorder 回退到 AIMD 声明，把没有默认值的字段视为必填，并接受 `fieldMeta` 覆盖。
