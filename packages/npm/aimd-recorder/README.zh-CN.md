...
把 Protocol 解析返回的 Pydantic JSON Schema 传给 Recorder，即可让 Python 模型成为唯一的校验契约：
...
Schema 契约同时支持 engine 的 `vars` / `steps` / `checks` 键和兼容用的 `research_variable` / `research_step` / `research_check` 别名，覆盖必填、类型、格式、pattern、枚举、数值边界、数组、对象及内建类型约束。对 Schema 标记为 required 且不可为空的 Recorder 输入，即使内存 Record 中已有该属性，空字符串和空数组仍会判为未填；可为空的属性接受 `null`，也不显示必填标记。Recorder 会先归一化上传文件等结构化值，再执行 Schema 校验。没有传 Schema 时，Recorder 回退到 AIMD 声明：没有默认值的字段视为必填，但类型包含 `None` 或使用 `Optional[...]` 的字段除外；AIMD 或 `fieldMeta` 中显式的 `required` 设置仍会生效。

默认在当前字段 `change` 和 `blur` 时即时校验，修正某个字段不会清除其他错误；可通过 `validationTriggers` 调整。`var_table` 错误使用 `var_table:samples:0:concentration` 这样的精确行列 key，错误展示和聚焦也会落到对应单元格。

`AimdRecorder` 和 `AimdRecorderEditor` 都会通过组件 ref 暴露 `validate()`、`validateField(fieldKey)`、`clearValidation(fieldKey?)` 和 `focusFirstInvalidField()`。提交前使用 `validate()` 作为前端拦截，它返回 `{ valid, issues, fieldState }`；通过后仍应执行服务端 Pydantic 校验。服务端错误可通过现有 `fieldState` 属性合并回填。
