...
The schema contract supports the engine's `vars` / `steps` / `checks` keys and the legacy `research_variable` / `research_step` / `research_check` aliases. It covers required, type, format, pattern, enum, numeric, array, object, and built-in-type constraints. Schema-required non-nullable recorder inputs reject empty strings and empty arrays even though the property already exists in the in-memory Record; nullable properties accept `null` and do not show a required marker. Structured recorder values such as uploaded files are normalized before schema validation. When no schema is supplied, Recorder falls back to AIMD declarations, treats fields without a default as required unless their type includes `None` or uses `Optional[...]`, and honors explicit `required` overrides from AIMD or `fieldMeta`.
