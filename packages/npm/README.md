...
| `@airalogy/aira-core` | [![npm version](https://img.shields.io/npm/v/%40airalogy%2Faira-core?logo=npm&color=cb3837)](https://www.npmjs.com/package/@airalogy/aira-core) | [README](./aira-core/README.md) | Browser-compatible parser, creator, and validator for Airalogy `.aira` archives |
...
Package boundaries: `@airalogy/aira-core` works at the `.aira` archive and Record payload layer: opening ZIP containers, reading manifests and files, validating archive references, and creating browser-side protocol archives. `@airalogy/aimd-core` works at the AIMD text layer: parsing `.aimd` Markdown, extracting protocol fields, and providing field-aware utilities for renderer, recorder, and host apps. Apps that open `.aira` files usually combine both packages: use `aira-core` to load archive contents, then use `aimd-core` to interpret the AIMD protocol text.
